...
public class GodRefreshLayout extends LinearLayout {
    ...
    private BaseRefreshManager baseRefreshManager;
    ...
    public void setRefreshManager() {
        baseRefreshManager = new DefaultRefreshManager(mContext);
        initHeaderView();
    }

    private void initHeaderView() {
        setOrientation(VERTICAL);
        mHeadView = baseRefreshManager.getHeaderView();
        mHeadView.measure(0, 0);
        mHeadViewHeight = mHeadView.getMeasuredHeight();
        minHeadViewHeight = -mHeadViewHeight;
        maxHeadViewHeight = (int) (mHeadViewHeight * 0.3f);
        Log.i("GodRefreshLayout:", " mHeadViewHeight: " + mHeadViewHeight + " minHeadViewHeight: " + minHeadViewHeight + " maxHeadViewHeight: " + maxHeadViewHeight);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (dy > 0) {
                    ...
                    if (topMargin <= 0) {
                        ...
                        baseRefreshManager.downRefreshPercent(percent);
                    }
    ...
    private void handleRefreshState(RefreshState mCurrentRefreshState) {
        switch (mCurrentRefreshState) {
            case IDDLE:
                baseRefreshManager.iddleRefresh();
                break;
            case REFRESHING:
                baseRefreshManager.refreshing();
                break;
            case DOWNREFRESH:
                baseRefreshManager.downRefresh();
                break;
            case RELEASEREFRESH:
                baseRefreshManager.releaseRefresh();
                break;
            default:
                break;
        }
